...
function cariModus(arr) {
  ...
  for(var i = 1; i < arr.length; i++){
    ...
    for(var j = 0; j < input.length; j ++){
      if(arr[i] === input[j]) same ++
    }
    if(same === 0) input.push(arr[i])
  }
  ...
  for(var i = 0; i < input.length; i++){
    ...
    for(var j = 0; j < arr.length; j++){
      if(arr[j] === input[i]) jumlah++
    }
    arrJumlah.push(jumlah)
  }
  
  if(arrJumlah.length === 1) return -1

  var addition = 0;
  for(var i = 0; i < arrJumlah.length; i++){
    addition += arrJumlah[i]
  }
  if(addition === arrJumlah.length) return -1

  var modus = arrJumlah[0];
  for(var i = 1; i < arrJumlah.length; i++){
    if(arrJumlah[i] > modus) modus = arrJumlah[i]
  }
  for(var i = 0; i < arrJumlah.length; i++){
    if(arrJumlah[i] === modus) return input[i]
  }
